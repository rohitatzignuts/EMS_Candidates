...
// set chip colors acc to the application status
const statusChipColor = (value: string) =>
	value === 'Accepted' ? 'success' : value === 'Rejected' ? 'error' : 'warning'
</script>
			...
			<div class="mb-4">
				<VRow>
					...
					<VCol cols="4" class="text-right">
						<!-- conditionaly show the apply button  -->
						<VChip
							v-if="router.fullPath === '/applications'"
							:color="statusChipColor(props.job.status)"
							variant="tonal"
							size="large"
							>{{ props.job.status }}
